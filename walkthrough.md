...
- **Replay State Syncing** ([replay.py](file:///c:/Users/Falak/Desktop/MyOnsite/backend/app/api/replay.py)): Updated the `/replay` and `/replay/fixture/{name}` endpoints to reset and apply the replayed transaction sequence to the global `state_engine`. This allows the dashboard panels (Transactions, Accounts, Visualizations, Anomalies) to reflect the replayed dataset correctly instead of remaining empty.
...
- **Windows Networking Compatibility** ([useWebSocket.js](file:///c:/Users/Falak/Desktop/MyOnsite/frontend/src/hooks/useWebSocket.js) & [api.js](file:///c:/Users/Falak/Desktop/MyOnsite/frontend/src/services/api.js)): Updated API base URLs and WebSocket client hostname configuration to map `localhost` to `127.0.0.1`. This resolves connection failures (e.g. "Stream disconnected") caused by dual-stack IPv6/IPv4 hostname resolution issues on Windows.
